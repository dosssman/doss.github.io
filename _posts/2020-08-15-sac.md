...
As a side experiment, the impact of the number of Q-network was empirically investigated.
...
[_Also, a note on having two optimizers in the SAC_]()
...
First, we analyse the result for the relatively simplest environment of the Mujoco robotic locomotion tasks suite.
{: .text-justify}
...
This would suggest that the automatic tuning mechanism allows to strike a good trade-off between policy randomness (useful for exploration) and optimality with respect to the objective (reward function).
{: .text-justify}
...
optimizer = optim.Adam(list(qf1.parameters()) + list(qf2.parameters()) + list(pg.parameters()), lr=<learning_rate>)
...
qf1_pi.requires_grad_(False), qf2_pi.requires_grad(False)
...
qf1_pi.requires_grad_(True), qf2_pi.requires_grad(True)
