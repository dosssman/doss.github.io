...
The default practice is to update the policy network every second update of the critic networks (the hyper parameter `--actor-update-interval=2`, code wise).
...
# At the time of writting, stable baselines did not seem to implement the L2 regularization for the means and logstd: 
...
This in turn would have led to its removal (comment out, to be more precise), which suggested that either said machine precision error was address in subsequent versions of TF, or that it had negligible impact on the results.
