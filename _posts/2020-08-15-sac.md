...
Therefore, by conditioning the *standard deviation* parameter of our Normal distribution, the agent can systematically control how much exploration (how far away from the optimal action) to do in the action space.
As an intuitive example, the *standard deviation* of the agent for a state that it has encountered often enough would be lower.
On the other hand, the *standard deviation* of the agent for a state that might have not been explore well enough yet will be larger, therefore making the agent behave less conservatively, and effectively exhausting the action space to build a better knowledge of that unexplored state.
{: .text-justify}
...
The entropy maximization objective is materialized by adding the ...
**TODO: insert the entroy augmented objective function of the agent following the one that was introduced in the default objective function in the RL background section. Also, design the pics.**
...
The Soft Q Value function is a transformation of the standard Q Value function into a policy by ... **TODO: Is this necessary ? Can we 
{: .text-justify}

**TODO: Better intuitive explanation**
